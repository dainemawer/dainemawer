...
It also puts the reviewee in the position of explaining their own logic, which tends to build confidence rather than dent it, and it usually gets you a clearer picture of what they were going for. That second question is worth asking often, since [returning the wrong data type](/using-the-return-statement) is the kind of bug that runs without erroring.
...
GitHub Actions can run these checks automatically on push or PR. Husky or lint-staged narrow that further, running lint scripts only against changed files, and [CommitLint does the same job for commit messages](/leveraging-commitlint). Between the two, most styling issues and careless mistakes never need a human eye at all.
...
Getting good at code review takes time for senior and junior engineers alike. Done well, it changes how a team ships, not just how clean the code looks. Open communication, feedback aimed at the code rather than the person, review that goes beyond the diff, and automation for everything that doesn't need a human, that's most of it. All of it still applies when [the author of the diff is an agent rather than a person](/reviewing-ai-generated-code), with one addition: you also have to ask whether the change should exist at all.
