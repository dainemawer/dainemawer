...
That adds up. Less CSS means a small but real performance win, and it's easier to read besides. CSS's newer [`if()` function pushes the same idea further](/css-if-function-safely), collapsing the condition itself into the declaration. If you want to poke at it yourself, here's a [CodePen](https://codepen.io/iamdainemawer/pen/NWmQZMz) to fork.
